...
class TaxComplianceSettingsScreen extends StatefulWidget {
  ...
  @override
  State<TaxComplianceSettingsScreen> createState() =>
      _TaxComplianceSettingsScreenState();
}

class _TaxComplianceSettingsScreenState
    extends State<TaxComplianceSettingsScreen> {
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _turnoverController.text =
          (prefs.getDouble('turnover_rate') ?? 4.0).toString();
      _levyController.text = (prefs.getDouble('levy_rate') ?? 1.5).toString();
      _incomeTaxController.text =
          (prefs.getDouble('income_tax_rate') ?? 30.0).toString();
      _withholdingTaxController.text =
          (prefs.getDouble('withholding_tax_rate') ?? 15.0).toString();
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      await prefs.setDouble(
        'vat_rate',
        double.tryParse(_vatController.text) ?? 16.0,
      );
      await prefs.setDouble(
        'turnover_rate',
        double.tryParse(_turnoverController.text) ?? 4.0,
      );
      await prefs.setDouble(
        'levy_rate',
        double.tryParse(_levyController.text) ?? 1.5,
      );
      await prefs.setDouble(
        'income_tax_rate',
        double.tryParse(_incomeTaxController.text) ?? 30.0,
      );
      await prefs.setDouble(
        'withholding_tax_rate',
        double.tryParse(_withholdingTaxController.text) ?? 15.0,
      );
