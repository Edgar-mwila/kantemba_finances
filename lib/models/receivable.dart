...
class Receivable {
  ...
  final String interestType; // 'fixed' or 'percentage'
  final double interestValue;
  final DateTime dueDate;
  final String paymentPlan; // e.g. 'installment', 'lump_sum'
  ...
  factory Receivable.fromJson(Map<String, dynamic> json) => Receivable(
    ...
    paymentHistory: (json['paymentHistory'] as List<dynamic>?)?.map((e) => ReceivablePayment.fromJson(e)).toList() ?? [],
    status: json['status'],
...
class ReceivablePayment {
  ...
  final DateTime date;
  final String method;
  ReceivablePayment({required this.amount, required this.date, required this.method});

  factory ReceivablePayment.fromJson(Map<String, dynamic> json) => ReceivablePayment(
